...
- **Database:** A relational database (PostgreSQL via Vercel Postgres, or SQLite for local dev) managed with **Prisma ORM**.
...
- `app/(tenant)/tenant/maintenance/page.tsx`: Maintenance request submission (description + image upload) and history.
...
- `app/(admin)/admin/properties/page.tsx`: Add/edit/remove properties.
...
- `app/(admin)/admin/maintenance/page.tsx`: Update maintenance request statuses.
...
- **Server Actions** will act as the primary API boundary for form submissions (e.g., `createApplication`, `submitMaintenance`, `processMockPayment`, `updatePropertyStatus`).
...
- No external third-party integrations are strictly required (Stripe is simulated via mock).

## 6. Ordered Implementation Strategy
1. **Infrastructure & Schema Initialization:**
   - Install Prisma, setup database (Postgres/SQLite).
...
5. **Admin Portal:**
   ...
   - Implement Property CRUD features.
...
- **Manual Verification (AC Checklist):**
  ...
  - Test the full lifecycle: User applies -> Admin approves -> Tenant logs in -> Tenant pays rent -> Tenant submits maintenance -> Admin resolves maintenance.
- **Visual Testing:** Check high contrast, responsive reflow of thick-bordered panels, and readability of the chosen fonts.
- **Functional Testing:** Ensure role-based access works (tenants cannot access `/admin`, public users cannot access `/tenant`).
