          ...
          <dl className="mt-8 space-y-5 text-sm text-muted">
            {contact.details.map((detail) => (
              <div key={detail.label}>
                ...
                <dd className="mt-2 text-base text-white">{detail.value}</dd>
              </div>
            ))}
          </dl>
        </SurfaceCard>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-panel p-8 shadow-soft">
          ...
          <form className="mt-8 grid gap-4">
            {contact.formFields.map((field) => (
              <label key={field.label} className="grid gap-2 text-sm text-muted">
                {field.label}
                {field.type === "textarea" ? (
                  <textarea rows={field.rows} placeholder={field.placeholder} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-muted/70" />
                ) : (
                  <input type="text" placeholder={field.placeholder} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-muted/70" />
                )}
              </label>
            ))}
